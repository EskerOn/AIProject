...
def fitness(toeval : Individual): 
    return toeval.Period*random.randint(0,50)/1000*3.5
...
class GeneticA():
    def __init__(self, MAXDEC, MAXEP, MINEP, NATRIB, NCLAS, MAXCAP, FITNESS):
        ...
        self.sumFitness=0
        self.maxFit=Individual(0,0,0,[])
        self.maxFit.FitnessValue=0
        #GA()
...
"""
    def maxim(self, population):
        ...
        for item in population:
            if item.FitnessValue>maxitem.FitnessValue:
                maxitem=item
        return maxitem
    def takefit(self, elem):
        return elem.FitnessValue
    def GA(self, Fitness_threshold : float, P : int, R : float, M : int, fitness):
        ...
        self.sumFitness= sum([i.FitnessValue for i in population])
        #for element in population:
        #    element.probability=element.FitnessValue//self.sumFitness
        population.sort(key=self.takefit, reverse=True)
        gen=0
        while(self.maxFit.FitnessValue<Fitness_threshold):
            print("Generation: " + str(gen) + " Fittest: " + str(self.maxFit.FitnessValue))
            #print("population: ", end="")
            #toprint=[element.FitnessValue for element in population]
            #print(toprint)

            sample=population[:int((1-R)*P)]
            #print("sample: ", end="")
            #toprint=[element.FitnessValue for element in sample]
            #print(toprint)
            #sample=random.sample(population, int((1-R)*P))
            for item in sample:
                #if(self.pr(item)): ###aqui va la probabilidad Pr(hi)
                populationS.append(item)
            topair=population[:int(R*P)]
            pairs=[(topair[i],topair[i+1]) for i in range(len(topair)-1)]
            ...
            sample=[]
            for i in range(int(len(populationS)*M)):
                sample.append(populationS.pop(random.randrange(len(populationS))))
            for item in sample:
                mutated = self.mutation(item.GeneticForm) ##se aplica mutación sobre item
                populationS.append(mutated)
            population=populationS
            for element in population:
                element.FitnessValue=fitness(element)
            population.sort(key=self.takefit, reverse=True)
            self.maxFit=self.maxim(population)
            gen+=1
        return self.maxFit

MAXDEC = 2 	# Cantidad de decimales en los float
MAXEP = 577  	# Máximo de etapas
...
val=gen.GA(100, 10, 0.4, 0.2, fitness)
print("finalizó max: " + str(val.FitnessValue))
"""
gen=GeneticA(MAXDEC, MAXEP, MINEP, NATRIB, NCLAS, MAXCAP, fitness)
#print(SD)
#ejemplos
